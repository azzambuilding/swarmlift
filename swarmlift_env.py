...
# Physical Constants
ARENA_SIZE = 20.0           # Arena spans [-10, 10] in x and y
MAX_THRUST = 1.0            # Maximum thrust magnitude per drone per axis
PAYLOAD_MASS = 3.0          # Total payload mass
PAYLOAD_INERTIA = 3.0       # Moment of inertia about the centroid
LINEAR_DAMPING =  0.4       # Damping on linear velocity (simulates drag)
ANGULAR_DAMPING = 0.2       # Damping on angular velocity
...
# Fixed asymetric triangle: three drone attach to the payload at fixed points.
# Scalene so equal thrust produce torque imbalance.
...
START_POS = np.array([-7.0, 0.0], dtype=np.float32)
END_POS = np.array([7.0, 0.0], dtype=np.float32)
...
class SwarmLiftEnv(ParallelEnv):
    ...
    def step(self, actions):
        ...
        world_offsets = TRIANGLE_LOCAL @ np.transpose(R)           # Shape (3,2)
        ...
        # Angular motion
        ang_accel = total_torque / PAYLOAD_INERTIA
        ...
        # Penalize distance from goal and angular velocity. Bonus for success.
        shared_reward = -0.01 * dist_to_goal - 0.001 * (self.payload_omega ** 2)   
        if reached_goal:
            shared_reward += 10.0
    ...
    def _drone_world_positions(self):
        ...
        R = rot_matrix(self.payload_theta)
        return self.payload_pos[None, :] + TRIANGLE_LOCAL @ np.transpose(R)
    ...
    def _drone_world_velocities(self):
        ...
        """
        R = rot_matrix(self.payload_theta)
        world_offsets = TRIANGLE_LOCAL @ np.transpose(R)
